...
laser_jobs = [j for j in jobs if j.get('router_name') == 'Laser Ruida'][:6]

print("=== RECENT LASER RUIDA JOBS IN DB ===")
for j in laser_jobs:
    print(f"ID #{j['id']} | file_name: '{j.get('file_name')}' | start: {j.get('start_time')} | end: {j.get('end_time')}")
    print(f"  dur: {j.get('duration_minutes')} min | est: {j.get('estimated_minutes')} min | area_m2: {j.get('bounding_area_m2')}")
