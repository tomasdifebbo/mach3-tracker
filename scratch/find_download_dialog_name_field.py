    ...
    def enum_win_cb(hwnd, lparam):
        if user32.IsWindowVisible(hwnd):
            ...
            if "download" in title.lower():
                ...
                edits.sort(key=lambda e: (e[3], e[2])) # sort by y, then x
                print(f"Edit controls in Download Document dialog ({len(edits)} edits):")
