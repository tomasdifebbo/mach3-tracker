    ...
    def enum_win_cb(hwnd, lparam):
        if user32.IsWindowVisible(hwnd):
            ...
            if "download" in title.lower():
                ...
                print(f"\n==============================================")
                print(f"Download Dialog HWND {hwnd}: '{title}' pos=({wx},{wy}) size=({ww},{wh})")
                print(f"==============================================")
                ...
                def enum_child_cb(chwnd, lparam):
                    ...
                    rel_x = cx - wx
                    rel_y = cy - wy
                    controls.append((chwnd, ccls, txt, rel_x, rel_y, cw, ch))
                    return True

                user32.EnumChildWindows(hwnd, WNDENUMPROC(enum_child_cb), 0)
                for chwnd, ccls, txt, rx, ry, cw, ch in controls:
                    print(f"  [HWND: {chwnd:7d}] [{ccls:10s}] rel_pos=({rx:4d},{ry:4d}) size=({cw:3d},{ch:3d}) text='{txt}'")
        return True
