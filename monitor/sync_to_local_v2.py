"""
Sync LOCAL v2 - Faz login local e restaura o banco.
"""
...
# Login Local
try:
    print(f"[*] Fazendo login local para {config['email']}...")
    ...
    if r.status_code == 200:
        ...
        print("[v] Login local realizado!")
    else:
        print(f"[!] Erro no login local: {r.status_code} - {r.text}")
        exit()
except Exception as e:
    print(f"[!] Erro ao conectar no servidor local: {e}")
    exit()
...
for router in routers:
    ...
    print(f"  Enviando {len(valid_jobs)} jobs...")
