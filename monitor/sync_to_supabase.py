"""
Sync SUPABASE - Restaura o histórico completo para o novo banco no Supabase.
"""
...
# Login Local (que agora salva no Supabase)
try:
    print(f"[*] Autenticando com o servidor...")
    ...
    if r.status_code == 200:
        token = r.json()["token"]
        print("[v] Autenticado com sucesso!")
    else:
...
for router in routers:
    ...
    print(f"  Enviando {len(valid_jobs)} trabalhos para nuvem Supabase...")
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                ...
                if j.get("end_time") and jid:
                    requests.patch(f"{BASE_URL}/api/jobs/{jid}", json={"end_time": j["end_time"]}, headers=headers, timeout=10)
                total_synced += 1
...
print(f"\nRESTAURO SUPABASE CONCLUIDO: {total_synced} jobs")
