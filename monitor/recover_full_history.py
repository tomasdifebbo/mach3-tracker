...
def simulate_gcode_time(filepath):
    ...
    unc_mappings = { 
        ...
        r"E:\arquivos 2024\ARQUIVOS 2026": r"E:\arquivos 2024\ARQUIVOS 2026"
    }
...
def recover():
    ...
    for line in lines:
        ...
        if status == "INICIO":
            ...
            # FILTRO 1: Ignora jobs fantasmas
            if duration_sec < 20: continue
            
            # REGRA DE OURO: Se durar mais de 2 horas (120 min), é erro de log.
            # Vamos forçar o tempo para a simulação do G-Code.
            duration_min = duration_sec / 60
            est = simulate_gcode_time(job['file_path'])
            
            if duration_min > 120:
                # Força o fim do job para ser Início + Estimativa
    ...
    # Deduplicação (evita 3 inicios seguidos do mesmo arquivo)
    final_jobs = []
    ...
    print(f"Subindo {len(final_jobs)} trabalhos higienizados...")
    ...
    for job in final_jobs:
        payload = {
            ...
            "material_price": 0 # Custo zero no histórico para não poluir
        }
        try:
            ...
            if r.status_code in (200, 201):
                ...
                count += 1
                if count % 50 == 0: print(f"{count} jobs restaurados...")
        except: pass
    print(f"Sucesso: {count} jobs com tempos e custos REAIS.")
